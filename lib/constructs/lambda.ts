...
export class FailoverLambda extends Construct {
  ...
  constructor(scope: Construct, id: string, props: FailoverLambdaProps) {
    ...
    const failureCountParameterName = `/infra-pipeline/${secondaryPipelineName}/consecutive-primary-failures`;
    const lastFailoverParameterName = `/infra-pipeline/${secondaryPipelineName}/last-failover-key`;
